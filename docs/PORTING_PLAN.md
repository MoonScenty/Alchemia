...
- [ ] **2. 상 시스템**
  ...
  - [x] E. 슬롯 위 상 아이콘 오버레이 (`client/AspectOverlay.java`). 원본과 같이 **고글 없이 Shift**로 표시되며, 설정 `alwaysShowAspects`로 뒤집을 수 있다.
    - 아직 인게임에서 눈으로 확인하지 않았다.
    - 지금은 모든 상이 그대로 보인다. 미발견 상을 물음표로 가리는 것은 3·4단계에서 붙인다 (아래 **미발견 상 가리기** 참고).
- [ ] **3. 플레이어 데이터** — 지식·워프 Data Attachment, 동기화 패킷, 명령어
  - **발견한 상 목록**을 여기에 넣는다. 오버레이가 이걸 보고 미발견 상을 물음표로 가린다.
  - 새 플레이어는 원시 6종을 아는 상태로 시작한다. 합성 상은 스캔으로 알아낸다.
